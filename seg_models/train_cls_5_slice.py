...
def epochVal(model, dataLoader, loss_seg, loss_cls, c_val, val_batch_size, cls_weight, seg_weight):
    ...
    with torch.no_grad():
        for i, (input, target_seg, target_cls) in enumerate(dataLoader):
            ...
            varOutput_seg, varOutput_cls = model.module(varInput)
...
def do_random_batch_mixup(model, input, masks, clss, loss_seg, loss_cls):
    ...
    lossvalue_seg = gm * loss_seg(varOutput_seg, masks.float()) + (1 - gm) * loss_seg(varOutput_seg, perm_mask.float())
...
def do_random_batch_cutmix(model, input, masks, clss, loss_seg, loss_cls):
    ...
    lossvalue_seg = loss_seg(varOutput_seg.float(), masks.float())
...
def train_one_model(fold_id, model, train_batch_size, val_batch_size, lr_init, total_epoch, use_pretrained, pretrained_path,
    ...
    for f_fold in range(1):
        ...
        # c_train = np.where((df_all['fold'] != num_fold))[0]
        # c_val = np.where((df_all['fold'] == num_fold))[0]
        ...
        model_ema = ModelEmaV2(model, decay=0.997, device='cuda')
        if use_pretrained:
        ...
        for epochID in range(0, trMaxEpoch):
            ...
            for batchID, (input, target_seg, target_cls) in enumerate(tqdm(train_loader)):
                if batchID == 0:
                    ss_time = time.time()
                optimizer.zero_grad()
                with autocast():
            ...
            if (epochID + 1) % 5 == 0 or epochID > 39 or epochID == 0:
                valLoss, valLoss_seg, valLoss_cls, best_thrr_with_no_mask, best_dicer_with_no_mask, best_thrr_without_no_mask, best_dicer_without_no_mask = epochVal(
                    model_ema, val_loader, loss_seg, loss_cls, c_val, val_batch_size, cls_weight, seg_weight)
            epoch_time = time.time() - start_time
            if (epochID + 1) % 5 == 0 and epochID > 15:
                torch.save({'epoch': epochID + 1, 'state_dict': model_ema.module.state_dict(), 'valLoss': valLoss},
...
if __name__ == '__main__':
    ...
    pretrained_model_path = configs["pretrained_model_path"] % fold_id
